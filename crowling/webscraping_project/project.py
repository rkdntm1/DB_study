...
def scrape_weather():
    ...
    pm25 = dust.find_all("dd")[1].get_text() #초세먼지
...
def scrape_it_news():
    ...
    for index, news in enumerate(news_list):
        ...
        img = news.find("img")
        if img:
            a_idx = 1 # img 태그가 있으면 1번째 이미지 태그의 정보를 씀
...
def scrape_english():
    ...
    print("(영어지문)")
    for sentence in sentences[len(sentences)//2:]: # 8문장이 있따고 가정. index 기준 4 7까지 잘라서 가져옴
        print(sentence.get_text().strip())
    print()
    print("(한글지문)")
    for sentence in sentences[:len(sentences)//2]: # index 기준 0~ 3까지
        print(sentence.get_text().strip())
